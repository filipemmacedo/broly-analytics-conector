...
- **WHEN** a PUT request is made with `apiKey` equal to the masked sentinel (`"••••••••<last4>"`)
...
- **THEN** the API key input displays only the last 4 characters preceded by bullet characters (e.g., `••••••••abcd`)
